...
const NARRATIVES = {
  ...
  mexico86:{name:"MEXICO 86",desc:"Lineker's Golden Boot and the Hand of God",emoji:"🌮",cat:"england",iconic:4,players:["Peter Shilton","Gary Lineker","Peter Beardsley","Terry Butcher","Glenn Hoddle","Bryan Robson","Ray Wilkins","Kenny Sansom","Trevor Steven","Chris Waddle","Gary Stevens","Steve Hodge","Peter Reid","Mark Hateley"]},
  spain82:{name:"SPAIN 82",desc:"Went home unbeaten — and still went home",emoji:"☀️",cat:"england",iconic:2,players:["Peter Shilton","Ray Wilkins","Bryan Robson","Kevin Keegan","Trevor Francis","Paul Mariner","Terry Butcher","Phil Neal","Mick Mills","Steve Coppell","Trevor Brooking","Graham Rix","Kenny Sansom"]},
  ...
  citydynasty:{name:"CITY DYNASTY",desc:"Guardiola's relentless blue machine",emoji:"🏙️",cat:"club",iconic:3,players:["Phil Foden","John Stones","Kyle Walker","Jack Grealish","Kalvin Phillips","Raheem Sterling","Fabian Delph","James Milner","Joe Hart","Rico Lewis","James Trafford","Nico O'Reilly"]},

  leedsrevie:{name:"LEEDS REVIE",desc:"Revie's ruthless, brilliant, bad-tempered Leeds",emoji:"⚔️",cat:"club",iconic:4,players:["Paul Reaney","Terry Cooper","Jack Charlton","Norman Hunter","Allan Clarke","Mick Jones","Paul Madeley","Trevor Cherry"]},
  ...
  shootout:{name:"THE SHOOTOUT CURSE",desc:"The men who stepped up from twelve yards — and the ones who missed.",emoji:"🎯",cat:"theme",iconic:4,players:["Stuart Pearce","Chris Waddle","Gareth Southgate","David Batty","Paul Ince","Darius Vassell","Steven Gerrard","Frank Lampard","Jamie Carragher","Ashley Young","Marcus Rashford","Jadon Sancho","Bukayo Saka","Ashley Cole","Owen Hargreaves","David Beckham","Harry Kane"]},
  gaffers:{name:"THE GAFFERS",desc:"Players who went on to take the dugout.",emoji:"🎩",cat:"theme",iconic:2,players:["Gareth Southgate","Kevin Keegan","Steve Bruce","Paul Ince","Tony Adams","Stuart Pearce","Alan Shearer","Frank Lampard","Steven Gerrard","Wayne Rooney","John Terry","Gary Neville","Glenn Hoddle","Bryan Robson","Tim Sherwood","Nigel Clough"]},
  centurions:{name:"THE 100 CLUB",desc:"A century of caps for England — the rarest club of all.",emoji:"💯",cat:"theme",iconic:3,players:["Peter Shilton","Wayne Rooney","David Beckham","Steven Gerrard","Bobby Moore","Ashley Cole","Bobby Charlton","Frank Lampard","Billy Wright","Harry Kane","Kyle Walker"]},
...
const ARCH_ERA_NAMES = {
  ...
  G: ["THE GOLDEN AGE","GOLDEN-ERA XI","PEAK YEARS XI","THE GLORY YEARS"],
...
const NARR_TAGLINES = {
  ...
  mexico86:["Lineker's boot. Maradona's hand.","Six goals, one robbery, one legend.","The summer England met the Hand of God."],
  spain82:["Unbeaten. Eliminated. Very England.","Didn't lose a game — didn't win the tournament.","Three clean sheets and a plane home."],
  leedsrevie:["Ruthless, brilliant, and loathed by everyone.","Revie's men took no prisoners.","Dirty Leeds — and utterly magnificent."],
  shootout:["Twelve yards. A nation holding its breath.","Some scored. Some never lived it down.","The walk every Englishman remembers."],
  gaffers:["They played. Then they took the dugout.","Captains, leaders, and future managers.","The ones who came back with a clipboard."],
  centurions:["A hundred caps. Almost nobody gets there.","Ten decades, eleven legends, one club.","Century-makers in the white shirt."],
...
const STORY_INFO = {
  ...
  citydynasty:{e:"🏙️",n:"CITY DYNASTY",tier:"common"},
  ...
  mexico86:{e:"🌮",n:"MEXICO 86",tier:"legendary"},
  spain82:{e:"☀️",n:"SPAIN 82",tier:"common"},
  leedsrevie:{e:"⚔️",n:"LEEDS REVIE",tier:"common"},
  shootout:{e:"🎯",n:"THE SHOOTOUT CURSE",tier:"legendary"},
  gaffers:{e:"🎩",n:"THE GAFFERS",tier:"common"},
  centurions:{e:"💯",n:"THE 100 CLUB",tier:"common"},
...
// PUB: policz wynik biezacego XI DOKLADNIE tak jak lockXI (w1+w2+w3+taktyka), bez pokazywania reveala.
// Zwraca liczbe albo null (gdy skład niepełny). Zero zmian w istniejacej logice — tylko eksport.
// PUB: odtworzenie przerwanego draftu po odswiezeniu strony.
// Eksport CZYSTO DODAJACY, jak xadScoreCurrent/xadStoryFor — nic nie zmienia w dzialaniu solo.
window.flGetSavedDraft = function(){ try { return flGetSavedDraft(); } catch(e){ return null; } };
window.flRestoreDraft  = function(b){ try { flRestoreDraft(b); return true; } catch(e){ return false; } };

// PUB: auto-dobor skladu na zerze timera draftu. Eksport CZYSTO DODAJACY — solo nietkniete.
// Dla kazdego PUSTEGO slotu bierze pierwszego jeszcze niewybranego z PULI: najpierw dokladna
// pozycja (pos), potem szeroka grupa (p) — ta sama regula, co reczny wybor zawodnika (linia ~410).
// Po dobraniu zapisuje draft (flSaveDraft), zeby istniejaca sciezka LOCK IN zapisala pelne XI.
// Brak pasujacego w puli -> slot zostaje pusty (0 za to miejsce), czyli lokalnie schodzi do (a).
function flAutoFillDraft(){
  try{
    if(typeof SLOTS==="undefined" || !SLOTS || typeof POOL==="undefined" || !POOL) return null;
    var used = {};
    for(var i=0;i<SLOTS.length;i++){ if(SLOTS[i].player && SLOTS[i].player.n) used[SLOTS[i].player.n]=true; }
    function firstFree(slot){
      var k,p;
      for(k=0;k<POOL.length;k++){ p=POOL[k]; if(p&&p.n&&!used[p.n]&&p.pos===slot.pos) return p; } // dokladna pozycja
      for(k=0;k<POOL.length;k++){ p=POOL[k]; if(p&&p.n&&!used[p.n]&&p.p===slot.p)     return p; } // szeroka grupa
      return null;
    }
    var filled=0;
    for(var s=0;s<SLOTS.length;s++){
      if(SLOTS[s].player) continue;
      var pl = firstFree(SLOTS[s]);
      if(pl){ SLOTS[s].player = pl; used[pl.n]=true; filled++; }
    }
    try{ flSaveDraft(); }catch(e){}   // zapis -> LOCK IN zobaczy dobrany sklad
    return { filled:filled, total:SLOTS.length, complete:SLOTS.every(function(x){return !!x.player;}) };
  }catch(e){ return null; }
}
window.flAutoFillDraft = function(){ return flAutoFillDraft(); };

window.xadScoreCurrent = function(){
  try{
    if(!DEALT) return null;
    var xiPlayers = SLOTS.map(function(s){ return s.player; });
    if(xiPlayers.length !== 11 || xiPlayers.some(function(p){ return !p; })) return null;
    var xiNames = xiPlayers.map(function(p){ return p.n; });
    var det = detect(xiNames);
    var score = w1(xiPlayers);
    if(det.main) score += w2(det.main, det.counts[det.main]);
    if(CFG.USE_W3) score += w3(xiPlayers, det.counts);
    var tac = 0;
    SLOTS.forEach(function(s){ if(s.p !== "GK" && s.player && s.player.pos === s.pos) tac++; });
    score += tac * 3;
...
window.xadStoryFor = function(xiPlayers){
  ...
  if(det && det.main){
    ...
    return {key:det.main, name:N.name, emoji:N.emoji, desc: narrTagline(det.main) || N.desc || "", rest: restClause(coreSet, null), core:[...coreSet], coreKind:"narr"};
  }
  ...
  if(a){
    ...
    else if(a.kind==="era"){ xiPlayers.forEach(p => { if(p.e===a.key) coreSet.add(p.n); }); }
    return {key:"arch_"+a.kind, name:a.name, emoji:a.emoji, desc: archTagline(a) || "", rest: restClause(coreSet, mainClub), core:[...coreSet], coreKind:"arch"};
  }
  const mr = mixedRest(xiPlayers);
  return {key:"mixed", name:"MIXED XI", emoji:"🌍", desc: narrTagline("mixed") || "Your own blend — no single story.", rest: mr ? capFirst(mr)+"." : "", core:[], coreKind:"mixed"};
};
function xadShareXI(){
